...
// Load the ingredients database — path relative to server/utils/
...
try {
    ...
    // Build the lookup table with aliases (matching original AI-model logic)
    Object.entries(INGREDIENT_DB).forEach(([name, entry]) => {
        const record = {
            ingredient: name,
            label: entry.risk > 0 ? "harmful" : "safe",
            severity: getSeverity(entry.risk),
            risk: entry.risk || 0,
            nova: entry.nova || 1,
            desc: entry.desc || "",
        ...
        LOOKUP[cleanText(name)] = record;

        if (entry.aliases) {
            entry.aliases.forEach(alias => {
                if (alias) LOOKUP[cleanText(alias)] = record;
            });
        }
    });
    console.log(`✅ [mlUtils] Loaded ${Object.keys(LOOKUP).length} ingredient mapping entries`);
} catch (err) {
    console.error("❌ [mlUtils] Failed to load ingredients_db.json:", err.message);
}
...
// 20-PHASE CONSUMPTION FREQUENCY ENGINE (ported from Python)
    ...
    ingredients.forEach(ing => {
        ...
        tokens.forEach(token => {
            ...
            if (LOOKUP[tokenClean] && !seen.has(LOOKUP[tokenClean].ingredient)) {
                ...
                // Refine label based on risk for frontend categorisation
