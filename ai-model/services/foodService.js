...
exports.detectFood = async (image) => {
  try {
    ...
    const prompt = `Analyze this food image. Pay special attention to distinct textures like puffed rice (Mamra) vs cooked rice.
    If it contains puffed rice, sev, and chutney, identify it as 'Bhel Puri' or 'Sev Puri' rather than 'Mixed Rice'.
    Recognize specific Indian Street Foods (Pani Puri, Dahi Puri, Chaat).
    List identified items in a JSON array format with confidence scores.
    Example format: [{'name': 'Bhel Puri', 'confidence': 0.95}, {'name': 'Sev', 'confidence': 0.90}]`;
